...
def my_job():
    ...
    # subscribers = list.subscriber.all()
    # print(Category.subscriber())
    # for list in Post.objects.all():
    #     i = []
    #     if list.date > last and list.date < now:
            # print(list.title)
            # print(list.content)
            # print(list.category_id)
            # i = i.append(list.category_id)
            # print(i)
    ...
    for u in User.objects.all():
        if len(u.category_set.all()) > 0:
            ...
            msg = EmailMultiAlternatives(
                ...
                body='Новые посты за неделю',  # это то же, что и message
